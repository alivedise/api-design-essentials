...
The `Accept` header in HTTP requests specifies the media types that are acceptable for the response. Proper usage of the `Accept` header is crucial in API design to ensure effective and flexible communication between clients and servers. This guideline outlines best practices for using the `Accept` header in API design, with references to RFC 9110, content negotiation, HTTP problem details (RFC 9457), and OpenAPI descriptions.

## Guidance

- SHOULD be requested by the client.
- The accepting criteria is
